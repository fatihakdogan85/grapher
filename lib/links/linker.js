    ...
    _initIndex() {
        if (Meteor.isServer) {
            ...
            if (this.linkConfig.index) {
                ...
                this.mainCollection.rawCollection().createIndex({ [field]: 1 }, options).catch((error) => {
                    // ignore IndexOptionsConflict error
                    if(error.code !== 85) {
                        throw error;
                    }
                });
                ...
                if (this.linkConfig.unique) {
                    ...
                    this.mainCollection.rawCollection().createIndex(
                        {
                            [field]: 1,
                        },
                        options
